        ...
        <div class="container">
          ...
          <div class="notify__wrapper">
            ...
            <div class="notify__content">
              ...
              <notify
                v-else
                :messages="messagesMainDir"
                @loadMore="loadMore"
                :btnDisabled="maxLength"
              />
            </div>
          </div>
        </div>
...
export default {
    ...
    mounted () {
      this.$store.dispatch('notify/getMessagesLazy');
    },
    computed: {
      ...mapState({
        loading: state => state.loading.loading,
        messages: state => state.notify.messages,
        notifyCount: state => state.notify.notifyCount,
        notifyIncrement: state => state.notify.notifyIncrement,
        error: state => state.error.notifyError
      }),
      messagesMainDir () {
        return this.$store.getters['notify/getMessagesMainDir'](this.notifyCount);
      },
      maxLength () {
        return this.$store.getters['notify/maxLength'];
      }
    },
    methods: {
      getNotifyLazy () {
      ...
      loadMore() {
        this.$store.dispatch('notify/addNotifyCount');
      }
    }
}
</script>
